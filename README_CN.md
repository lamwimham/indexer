...
- [ ] **消息队列集成** - 引入消息队列 (BullMQ/Redis Streams)，解耦区块获取与事件处理，支持水平扩展和背压控制
- [ ] **缓存层** - 基于 Redis 添加热点数据缓存（最新区块、常用地址查询），减轻数据库压力，提升 API 响应速度
- [ ] **WebSocket 支持** - 实现 WebSocket API，支持实时事件推送、同步状态更新、前端订阅
